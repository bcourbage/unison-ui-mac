...
/// contract re-enables it. Each drives the cycle with an `.init2Completed`
/// source event (the only terminal that maps to a clean cause and may advance
/// toward a reusable `.stopped`); the fail-closed `.scanFailed` / `.genericFatal`
/// cases, and the event→cause mapping itself, live in
...
final class EngineScanInterruptionTests: XCTestCase {
    ...
    func test_foldedQueued_routesToFreshSession_notStopped() {
        ...
        _ = c.requestScanInterruption(s, destination: .stopInPlace)
        _ = c.transportSignalCompleted(s, op, .signalled(idA)); _ = c.interruptTerminalObserved(s, op, event: .init2Completed)
    ...
    func test_terminalBeforeSignalResult_thenSignalled_awaitsReap() {
        let (c, s, op) = scanning(); _ = c.requestScanInterruption(s, destination: .stopInPlace)
        XCTAssertTrue(c.interruptTerminalObserved(s, op, event: .init2Completed).isEmpty)     // remembered
    ...
    func test_noChild_restartRequired_evenIfTerminalRaced() {
        let (c, s, op) = scanning(); _ = c.requestScanInterruption(s, destination: .stopInPlace)
        _ = c.interruptTerminalObserved(s, op, event: .init2Completed)                       // terminal seen
    ...
    func test_deadline_signallingClass_firesAfterEarlyTerminalFlip() {
        let (c, s, op) = scanning(); _ = c.requestScanInterruption(s, destination: .stopInPlace)
        _ = c.interruptTerminalObserved(s, op, event: .init2Completed)                   // flag flips false→true
    ...
    func test_staleTerminalAfterStopped_isNoOp() {
        ...
        _ = c.interruptTerminalObserved(s, op, event: .init2Completed)                  // stale
        XCTAssertEqual(c.phase, .stopped(s))
    }
    ...
    private func assertStaleInterruptEventsInert(_ c: C, _ s: SID, _ op: OID,
        ...
        let before = c.phase
        XCTAssertTrue(c.interruptTerminalObserved(s, op, event: .init2Completed).isEmpty, file: file, line: line)

...
///   - `.unsafe` (scanFailed / generic fatal, no authenticated cause) → restart-
///     required from ANY interrupt stage, never `.stopped`.
///   - `.cleanCompletion` (init2-complete) → safe wind-down, applied exactly once.
/// It also proves the honest Return-to-Profiles abandon path retains the scan
/// lease: a second profile stays queued and archive maintenance stays forbidden
/// until the abandoned scan's own terminal and a successful close.
@MainActor
final class EngineScanInterruptTerminalCauseTests: XCTestCase {
    ...
    func test_scanFailedDuringInterruption_restartRequired_neverStopped() {
        ...
        let e = c.interruptTerminalObserved(
            s, op, cause: .unsafe("scan-failed during interruption"))
    ...
    func test_unsafeTerminal_atAwaitingReap_restartRequired() {
        ...
        _ = c.interruptTerminalObserved(s, op, cause: .cleanCompletion)  // → awaitingReap(idA)
        guard case .interruptingScan(_, _, .awaitingReap, _) = c.phase else {
            return XCTFail("precondition awaitingReap, got \(c.phase)")
        }
        _ = c.interruptTerminalObserved(s, op, cause: .unsafe("late fatal"))
        XCTAssertTrue(c.isRestartRequired)
    ...
    func test_cleanCompletion_safe_andExactlyOnce() {
        ...
        let e1 = c.interruptTerminalObserved(s, op, cause: .cleanCompletion)
        ...
        let e2 = c.interruptTerminalObserved(s, op, cause: .cleanCompletion)

...
| Size | 5.5 MB (5480232 bytes) |
| Built from | builder commit `1bf5d8b` on `feat/cli-session-engine-integration`, workflow run <https://github.com/bcourbage/unison-ui-mac/actions/runs/34675204517> (provenance manifest attached to that run). Note: this maintainer run reports overall `failure` because a post-upload XcodeGen-install step could not find its script and the in-CI smoke test was skipped; every blob step (build, symbol/patch-marker/size assertions, `minos 15.0`, provenance, artifact upload) succeeded BEFORE that step, and the app build + full test suite pass in the PR's own CI. |
| Reproducibility | Source, patch set (0002+0003+0004+0005+0006+0007+0008), toolchain (OCaml 5.5.0), deployment target (15.0), and build command above are all pinned. The resulting `.o` is **not byte-identical** across clean rebuilds on this toolchain: observed differing SHA-256 between two same-source builds (OCaml/`ld -r` output is not deterministic here). We therefore do NOT claim a bit-reproducible blob; we pin every input and record the exact checksum of the committed artifact. |

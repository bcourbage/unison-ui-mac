...
| Size | 5.5 MB (5487544 bytes) |
| Built from | builder commit `5bfac95` on `feat/cli-launch-arg-delivery`, workflow run <https://github.com/bcourbage/unison-ui-mac/actions/runs/34710421095> (provenance manifest attached to that run). This is the first run on the repaired workflow: it completed fully green, including the post-upload XcodeGen install and the build+test-against-the-fresh-blob smoke step. |
| Reproducibility | Source, patch set (0002+0003+0004+0005+0006+0007+0008+0009), toolchain (OCaml 5.5.0), deployment target (15.0), and build command above are all pinned. The resulting `.o` is **not byte-identical** across clean rebuilds on this toolchain: observed differing SHA-256 between two same-source builds (OCaml/`ld -r` output is not deterministic here). We therefore do NOT claim a bit-reproducible blob; we pin every input and record the exact checksum of the committed artifact. |

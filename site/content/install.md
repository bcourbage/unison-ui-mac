...
Homebrew installs create the `unison` link automatically. For a direct download,
**Settings ▸ Command Line** can add the app's bundled `unison` to your login-shell
PATH with no administrator password: it shows what `unison` currently resolves to,
writes a marked block to your shell startup file when it can do so safely, and
otherwise shows exactly what to add and where. The app also offers this at first
launch when nothing on your PATH is named `unison`. Details, and the note for
